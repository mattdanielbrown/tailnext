    ...
    <div className="mx-auto max-w-7xl">
      <div className={`md:flex ${isReversed ? 'md:flex-row-reverse' : ''} md:gap-16`}>
        <div className="self-center md:basis-1/2">
          {content && <div className="mb-8 text-lg text-gray-600 lg:mb-12 dark:text-slate-400">{content}</div>}
          <ItemGrid
            items={items}
            columns={1}
            ...
            iconClass="shrink-0 w-7 h-7 flex items-center justify-center rounded-full bg-primary-900 text-gray-50 mr-4 rtl:mr-0 rtl:ml-4 mt-1 p-1"
          />
        </div>
        <div aria-hidden="true" className="mt-10 md:mt-0 md:basis-1/2">
          {image && (
            <div className="relative m-auto max-w-4xl">
              <Image
                className="mx-auto w-full rounded-lg bg-gray-400 shadow-lg dark:bg-slate-700"
                src={image.src}
                width={828}
                height={828}
                alt={image.alt}
